...
public class DeleteRoomSO extends GeneralSystemOperation<GenericList<GeneralDomain>> {
	...
	protected void runConcreteSO(GenericList<GeneralDomain> object) throws SQLException, Exception {
		...
		Cache.getInstance().deleteFromMap(property.getName(),room);
		Cache.getInstance().deleteFromMap(property.getName(),room_info);
